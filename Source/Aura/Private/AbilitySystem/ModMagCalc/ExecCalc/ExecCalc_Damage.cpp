...
// Get damage set by caller magnitude.
	...
	for (const TPair<FGameplayTag, FGameplayTag>& Pair  : FAuraGameplayTags::Get().DamageTypesToResistances)
	{
		...
		checkf(AuraDamageStatics().TagsToCaptureDefs.Contains(ResistanceTag),
			TEXT("line 106: Tags to capture def doesn't contain tag: [%s] in ExecCalc_Damage"), *ResistanceTag.ToString());
	...
	const float CriticalHitResistCoX = CriticalHitResistanceCurve->Eval(TargetPlayerLevel);
